...
const VerifyUser = catchAsync(async (req: Request, res: Response) => {
    ...
    // Debug logs
    console.log('Verifying user with userId:', userId);
    console.log('Token:', uniqueString);
    
    const isUserExist = await prisma.doctor.findUnique({
        where: { id: userId }
    });
    
    console.log('Doctor found:', isUserExist);
    
    if (!isUserExist) {
        throw new ApiError(httpStatus.NOT_FOUND, "User is not found !!");
    }
    const getVerificationUser = await prisma.userVerification.findFirst({
        where: {
            userId: userId
        }
    })
